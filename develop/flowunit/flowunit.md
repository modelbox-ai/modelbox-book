...
流单元接口，包含流单元`插件初始化接口`，`流单元初始化接口`和`数据处理接口`。

### API接口类型实现对照关系

流单元提供了FlowUnit::相关的接口，其接口按不同类型的流单元而不同，开发者应根据FlowUnit处理数据的类型，选择实现相关的接口，对应的关系表如下：

|接口|接口类型|接口功能|调用实时机|是否必须|通用流单元|条件流单元|数据流流单元|数据流拆分流单元|数据流合并流单元|
|--|--|--|--|--|--|--|--|--|--|
|DriverInit|插件初始化接口|模块初始化|插件加载时调用一次|否|✔️|✔️|✔️|✔️|✔️|
|DriverFini|插件初关闭接口|模块退出|插件结束时调用一次|否|✔️|✔️|✔️|✔️|✔️|
|FlowUnit::Open|流单元初始化接口|流单元初始化|图加载流单元时调用|否|✔️|✔️|✔️|✔️|✔️|
|FlowUnit::Close|流单元关闭接口|流单元关闭|图结束时调用|否|✔️|✔️|✔️|✔️|✔️|
...
1. 大部分情况下，业务都属于通用流单元，仅需要处理单个数据，开发只需要实现FlowUnit::Process的功能即可。
1. 若流单元需要处理流数据，或需要记录状态，对数据进行跟踪处理，则需要实现FlowUnit:DataPre, FlowUnit::Process, FlowUnit::DataPost接口的功能。
...
对应需实现的接口为`FlowUnit::Open`、`FlowUnit::Close`，此接口可按需求实现。例如，使用::Open接口获取用户在图中的配置参数，使用::Close接口释放流单元的一些公共资源。
...
在业务处理过程中对数据进行拆分，然后在后续流单元中处理，当数据处理完成后需要对数据进行合并得到最终结果。
...
流单元上下文包含，`会话上下文|SessionContext`和`数据上下文|DataContext`
...
例如http服务同步响应场景，首先接收到http请求后转化成buffer数据，然后通过ExternalData->GetSessionContext接口获取到SessionContext，接着调用SessionContext->SetPrivate设置响应的回调函数，之后通过ExternalData->Send接口把buffer数据发送到flow中；经过中间的业务处理流单元；最后http响应流单元中在业务数据处理完成后，再调用SessionContext->GetPrivate获取响应回调函数，发送http响应。至此SessionContext也结束。

### DataContext 数据上下文

DataContext是提供给当前流单元处理数据时的临时获取BufferList
流单元处理一次Stream流数据，或一组数据的上下文，当数据生命周期不再属于当前流单元时，DataContext生命周期也随之结束。
